...
if (_isForceDespawn) then {
	...
	if (A3XAI_debugLevel > 0) then {diag_log format["A3XAI Debug: No players remain in %1. Deleting spawned AI in %2 seconds.",triggerText _trigger,A3XAI_despawnRandomSpawnTime];};
...
if (_canDespawn) then {
	...
	if (A3XAI_debugLevel > 0) then {diag_log format ["A3XAI Debug: A player has entered the trigger area at %1. Cancelling despawn script.",(triggerText _trigger)];}; //Exit script if trigger has been reactivated since A3XAI_despawnRandomSpawnTime seconds has passed.
